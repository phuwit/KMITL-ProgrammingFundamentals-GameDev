...
void ColorScale::fillTab(sf::Color* colorTab, int size,InterpolationFunction::InterpolationFunction function) const
{
	...
	while(it!=last)
	{
		...
		{
			colorTab[i].r = (unsigned char)pFunction(startColor.r,endColor.r,fabs((double)i-pos)/(nb_color-1.0));
			colorTab[i].g = (unsigned char)pFunction(startColor.g,endColor.g,fabs((double)i-pos)/(nb_color-1.0));
			colorTab[i].b = (unsigned char)pFunction(startColor.b,endColor.b,fabs((double)i-pos)/(nb_color-1.0));
			colorTab[i].a = (unsigned char)pFunction(startColor.a,endColor.a,fabs((double)i-pos)/(nb_color-1.0));
		}
		pos+=nb_color;
	}
